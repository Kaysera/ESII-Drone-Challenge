classdef GazeboSpawnedModel < handle
    %GazeboSpawnedModel - Class controlling user-spawned models in Gazebo
    %   M = GazeboSpawnedModel(NAME,WORLD) creates an object that allows for easy
    ...
    %
    %   GazeboSpawnedModel properties:
    %       Name                    - Name of the Model
    %       Links                   - List of Link Names
    %       Joints                  - List of Joint Names
    %
    %   See also GazeboCommunicator, GazeboModel
    ...
    methods
        ...
        function obj = GazeboSpawnedModel(Name, World)
            %GazeboSpawnedModel Constructor
        ...
        function applyForce(obj, linkname, duration, varargin)
            ...
            %---------------
%            serviceMsg.ReferenceFrame = [obj.Name '::' linkname];

%            serviceMsg.ReferencePoint.X = 10;
%            serviceMsg.ReferencePoint.Y = 10;
%            serviceMsg.ReferencePoint.Z = 10;

            %---------------
            sec  = floor(duration);
            nsec = (duration - sec) * 1E9;
            serviceMsg.Duration.Sec  = int32(sec);
            serviceMsg.Duration.Nsec = int32(nsec);
